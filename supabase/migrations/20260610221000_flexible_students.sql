-- Migration: Modify students table to allow independent IDs and auth linking on registration

-- 1. Safely drop foreign keys and recreate the tables with new references
...
-- 2. Recreate Students table with separate primary key and optional auth_id
...
-- 3. Recreate Github Stats table referencing students(id)
...
-- Update profile policy: Users can only update their own student record mapped by auth_id
...
-- 8. Updated trigger to automatically link or create student records when GitHub users sign up
...
BEGIN
  ...
  -- Check if student already exists by github_username (case-insensitive)
  SELECT id INTO existing_id FROM public.students WHERE LOWER(github_username) = LOWER(username);
  
  IF existing_id IS NOT NULL THEN
    -- Link authentication to the existing student record
    UPDATE public.students 
    SET auth_id = new.id,
        avatar_url = COALESCE(students.avatar_url, new.raw_user_meta_data->>'avatar_url'),
        nombre = COALESCE(students.nombre, new.raw_user_meta_data->>'full_name', new.raw_user_meta_data->>'name')
    WHERE id = existing_id;
  ELSE
    -- Insert a new student record
    INSERT INTO public.students (auth_id, nombre, github_username, avatar_url)
    VALUES (
      new.id,
